  ...
  if request.session['adu']:
   if request.method == 'POST':
      ...
      if ch:
       ...
       print('oaky 1')
       user = User.objects.create_user(username=usr,password=psd,first_name=name,last_name=bus)
       user.save()
       print('oaky 2')
       cu = User.objects.get(username=usr)
       pro = Profile(counter_ip= cip , kitchen_ip= kip , pro_usr = cu,live = False, mobile = mob ,email = em,business_name = bus,address =add)
       pro.save()
       print('oaky 3')
